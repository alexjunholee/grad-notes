...
메일은 짧고 답하기 쉬운 단위만 담는 도구다. 길게 설명할 내용은 직접 만남으로 옮긴다 — 권창현 교수의 명시 권고다. 메일에서 길게 설명하면 받는 사람의 인지 부하가 다시 커지고, 결국 답이 늦거나 빠진다.

서로 다른 주제는 별도 메일로 보낸다. 한 메일에 두 주제가 섞이면 한 주제는 답이 안 돌아온다. 첫 주제에서 답이 끝났다고 받는 사람이 send를 누르고, 두 번째 주제는 머리에서 휘발된다. 메일을 다시 보내면 방금 보냈는데 또 보내네의 인상이 남는다. 별도 메일로 처음부터 보낸 경우와는 다른 인상이 남는다.

> 길게 설명할 내용은 메일이 아닌 만남으로 옮긴다. 서로 다른 주제는 별도 메일로 보낸다.

메일 한 통이 같이 일하는 단위다. 박사 5년 동안 교수와의 협업은 수백 통의 메일 위에 굴러가고, 한 통의 단가를 줄이는 일은 5년 누적에서 교수가 같이 일하기 편한 학생이라는 평판으로 돌아온다. 이 장의 한 줄 정의(§1)가 거기서 회수된다.

---

이메일은 도구다. 도구의 단가가 학생 평가에 직접 옮겨 적힌다 — 이 장의 관점이다. 도구의 단가를 줄이는 학습이 박사 5년의 평판을 한 단위씩 끌어올린다. 다음 챕터는 도구의 단가가 반대 방향으로 어떻게 커지는가를 본다 — 도구를 늘리는 일이 그 자체로 진척처럼 느껴지는 함정.

**출처.** [권창현 교수 「대학원생의 이메일 커뮤니케이션」](https://gradschoolstory.chkwon.net/changhyun/email-communication/)이 1차. 좋은 학생 정의·한 메일 한 질문·형식 표준·좋은 메일 제목과 나쁜 메일 제목·본문 원문 인용, 모두 여기서 가져왔다. 좋은 메일의 본문 sample은 같은 배경 → 시도 → 질문 관점에 맞춰 재구성. R2 cognitive load 1-sentence cite는 [Mensh & Kording 2017 rule 2](https://doi.org/10.1371/journal.pcbi.1005619), 자세한 관점은 「연구노트」 측 정독. robotics-practice ch21 § 생존 마인드셋 — "안 돼요"는 보고가 아니다의 분야 쪽 논의. "메일이 학생 평가의 가시 지표"라는 관점과 "메일 한 통 = 같이 일하는 단위" 표현은 이 장에서 정리했다.
