...
코드가 안 도는 것 정상은 robotics-practice 본체가 맡는다. 여기서는 처음 읽는 논문이 이해되지 않는 상태만 다룬다.
...
> 첫 주의 운영은 환경 → 코드 → 논문 → 방향 시퀀스로 잘라 본다. 분야 도구의 셋업 목록은 robotics-practice ch21 § E에 위임하고, 여기서는 그 순서만 남긴다.
